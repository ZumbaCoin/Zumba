...
const char * const BITCOIN_CONF_FILENAME = "zumba.conf";
const char * const BITCOIN_PID_FILENAME = "zumbad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zumba" is a composite category enabling all Zumba-related debug output
            if(ptrCategory->count(string("zumba"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZumbaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZumbaCore
    // Mac: ~/Library/Application Support/ZumbaCore
    // Unix: ~/.zumbacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZumbaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZumbaCore";
#else
    // Unix
    return pathRet / ".zumbacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zumba.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zumba.conf
